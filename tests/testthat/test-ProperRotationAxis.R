...
test_that("ProperRotationAxis constructs with integer Cn", {
  ...
  expect_type(ax@n, "integer")
  expect_equal(ax@n, 3L)
})

test_that("ProperRotationAxis coerces integer-like numerics to integer", {
  ax1 <- ProperRotationAxis(n = 3, posA = c(0,0,0), posB = c(0,0,1))     # double 3
  ax2 <- ProperRotationAxis(n = 3.0, posA = c(0,0,0), posB = c(0,0,1))   # double 3.0
  expect_type(ax1@n, "integer")
  expect_type(ax2@n, "integer")
  expect_identical(ax1@n, 3L)
  expect_identical(ax2@n, 3L)
})

test_that("ProperRotationAxis rejects true decimals", {
  # browser()
  expect_error(
    ProperRotationAxis(n = 1.1, posA = c(0,0,0), posB = c(0,0,1)),
    "must be a whole number, not",
    ignore.case = TRUE
  )
  expect_error(
    ProperRotationAxis(n = 2.7, posA = c(0,0,0), posB = c(0,0,1)),
    "must be a whole number, not",
    ignore.case = TRUE
  )
})
...
test_that("Cn must be length 1, finite, and >= 1", {
  expect_error(ProperRotationAxis(n = integer(0), posA = c(0,0,0), posB = c(0,0,1)),
               "single value|length", ignore.case = TRUE)
  expect_error(ProperRotationAxis(n = c(1L,2L), posA = c(0,0,0), posB = c(0,0,1)),
               "single", ignore.case = TRUE)
  expect_error(ProperRotationAxis(n = NA_integer_, posA = c(0,0,0), posB = c(0,0,1)),
               "non-NA|>= 1", ignore.case = TRUE)
  expect_error(ProperRotationAxis(n = 0L, posA = c(0,0,0), posB = c(0,0,1)),
               ">= 1", ignore.case = TRUE)
  expect_error(ProperRotationAxis(n = -2L, posA = c(0,0,0), posB = c(0,0,1)),
               ">= 1", ignore.case = TRUE)

  expect_error(ProperRotationAxis(n = Inf, posA = c(0,0,0), posB = c(0,0,1)),
               "finite", ignore.case = TRUE)
})

test_that("posA / posB length and finiteness validated", {
  expect_error(ProperRotationAxis(n = 2L, posA = c(0,0), posB = c(0,0,1)),
               "posA.*length 3", ignore.case = TRUE)

  expect_error(ProperRotationAxis <- ProperRotationAxis(n = 2L, posA = c(0,0,0), posB = c(0,0)),
               "posB.*length 3", ignore.case = TRUE)
  expect_error(ProperRotationAxis(n = 2L, posA = c(NA,0,0), posB = c(0,0,1)),
               "must have no missing values", ignore.case = TRUE)
  expect_error(ProperRotationAxis(n = 2L, posA = c(0,0,0), posB = c(0,Inf,1)),
               "must have no infinite values", ignore.case = TRUE)
})

test_that("posA and posB must be distinct", {
  expect_error(
    ProperRotationAxis(n = 2L, posA = c(0,0,0), posB = c(0,0,0)),
...
test_that("setter works when assigning after construction", {
  ...
  ax@n <- 6.0
  expect_identical(ax@n, 6L)
  # reject decimal
  expect_error({ ax@n <- 2.5 }, "must be a whole number", ignore.case = TRUE)
})
...
test_that("ProperRotationAxis label: defaults to a single, non-empty character", {
  ax <- ProperRotationAxis(n = 2L, posA = c(0,0,0), posB = c(0,0,1))  # no label supplied
...
test_that("ProperRotationAxis label: accepts and stores a custom label", {
  ax <- ProperRotationAxis(n = 3L, posA = c(0,0,0), posB = c(0,0,1), label = "principal_z")
  expect_identical(ax@label, "principal_z")
})

test_that("ProperRotationAxis label: can be updated via set_props and revalidated", {
  ax  <- ProperRotationAxis(n = 3L, posA = c(0,0,0), posB = c(0,0,1), label = "z1")
...
test_that("ProperRotationAxis label: rejects invalid values", {
  # empty string
  expect_error(
    ProperRotationAxis(n = 2L, posA = c(0,0,0), posB = c(0,0,1), label = ""),
  ...
  # NA
  expect_error(
    ProperRotationAxis(n = 2L, posA = c(0,0,0), posB = c(0,0,1), label = NA_character_),
  ...
  # length > 1
  expect_error(
    ProperRotationAxis(n = 2L, posA = c(0,0,0), posB = c(0,0,1), label = c("a","b")),
  ...
  expect_error(
    ProperRotationAxis(n = 2L, posA = c(0,0,0), posB = c(0,0,1), label = 123),
...
test_that("ProperRotationAxis label: preserved by transform_symmetry_axis", {
  ax <- ProperRotationAxis(n = 2L, posA = c(0,0,0), posB = c(0,0,1), label = "axis_A")
...
test_that("translation transform updates endpoints and preserves Cn", {
  ...
  expect_identical(ax2@n, 3L)                 # Cn unchanged
...
test_that("rotation around Z by 90 deg moves X-axis endpoints as expected", {
  ...
  ax <- ProperRotationAxis(n = 2L, posA = c(1,0,0), posB = c(2,0,0))
...
test_that("as.data.frame(ProperRotationAxis) returns a single-row data.frame with expected columns and types", {
  ax <- ProperRotationAxis(n = 3L, posA = c(0, 1, 2), posB = c(3, 4, 5), label = "axis-A")
  df <- as.data.frame(ax)
...
test_that("as.data.frame(ProperRotationAxis) respects numeric Cn input via coercion and preserves label", {
  ...
  ax <- ProperRotationAxis(n = 4, posA = c(1, 0, 0), posB = c(0, 1, 0), label = "axis-B")
  df <- as.data.frame(ax)
