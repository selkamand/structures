...
read_mol2 <- function(path){
  ...
  ls_contents_substplit_named <- lapply(ls_contents_substplit, FUN = function(v){tail(v, n=-2)})

  names(ls_contents_substplit_named) <- sub(x=names(ls_contents_substplit_named), pattern = "<TRIPOS>", replacement = "")
  return(ls_contents_substplit_named)
}
...
fileName <- 'inst/benzene.mol2'
contents <- readr::read_file(fileName)
ls_contents <- unlist(strsplit(contents, split = "@"))
ls_contents <- Filter(function(v){ f = nchar(v) > 0}, x=ls_contents)
ls_contents_substplit <- lapply(ls_contents, function(x){
  val <- unlist(strsplit(x, split =  "\n"));
...
names(ls_contents_substplit) <- sub(x=names(ls_contents_substplit_named), pattern = "<TRIPOS>", replacement = "")
return(ls_contents_substplit)
